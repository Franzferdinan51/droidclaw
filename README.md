...
## 🦆 duck-cli Integration (Franzferdinan51 fork)

This fork adds **duck-cli provider integration** — routes LLM calls through duck-cli's model stack.

**The perceive→reason→act loop is also available in [duck-cli](https://github.com/Franzferdinan51/duck-cli):**
```bash
cd duck-cli && ./duck android agent "open settings"
```

This fork (Franzferdinan51/droidclaw) tracks upstream. The main development home is **duck-cli**.
